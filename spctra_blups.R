...
ggplot(bands.H2, aes(band ,H2)) +
  ...
  labs(title = 'Broad Sense Heritability of the Bands')
          ...
          ################Calculating Blups for all predicted traits ######################


spectra <- read.csv("Spectrum_with_biochemical_all_predicted.csv")
spectra <- spectra[spectra$Group == 'Inbred', ]
spectra <- spectra %>% select(genotype, Trt, Group, Rep, SLA, LWC, X..N, X..P, X..K, X..S, X..Ca, X..Mg, ppm.Zn, ppm.Fe, ppm.Mn,
                              ppm.Cu, ppm.B, ppm.Mo)
...
blups_HN <- spectra.blups.list[['HN']]
colnames(blups_HN)[-1] <- paste(colnames(blups_HN)[-1], 'HN', sep = '_')
blups_LN <- spectra.blups.list[['LN']]
colnames(blups_LN)[-1] <- paste(colnames(blups_LN)[-1], 'LN', sep = '_')

blups_all_predicted <- merge(blups_HN, blups_LN, all = TRUE)

write.csv(blups_all_predicted, './blups_all_predicted.csv', row.names = FALSE)
